...
const CSV_PATH = 'data/jobs_dirty_A.csv';
...
//    큰따옴표로 묶인 필드 안의 콤마를 안전하게 처리한다 (예: "3년~7년, 프로모션 전담").
// ---------------------------------------------------------------------------

function stripBOM(text) {
  // 파일 맨 앞에 UTF-8 BOM(0xFEFF)이 붙어 있으면 제거한다.
...
function normalizeForDedup(value, options = {}) {
  ...
  normalized = normalized
    .replace(/㈜|\(주\)|주식회사|주\)|주\./g, '')
    .replace(/[!"#$%&'()*+,\-./:;<=>?@[\\\]^_`{|}~]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();

  if (options.companyName) {
    const companyName = normalizeForDedup(options.companyName);
    if (companyName && normalized.startsWith(companyName + ' ')) {
...
function getDedupKey(row) {
  const companyKey = normalizeForDedup(row.company);
  const titleKey = normalizeForDedup(row.title, {
    removeBrackets: true,
...
function removeDuplicates(rows) {
  ...
  for (const row of rows) {
    ...
    if (seenKeys.has(key)) {
      continue; // 이미 같은 공고를 남겼으므로 이번 행은 버린다
    }
    seenKeys.add(key);
    kept.push(row);
  }
  return kept; // 새 배열 - rows(원본)는 그대로
}
...
function cleanData(rows) {
  return preserveValues(removeDuplicates(rows)); // 새 객체이지만 원본 값은 그대로
}

// ---------------------------------------------------------------------------
// 3. D04 필터 함수 3개 (지역 / 경력 조건(category) / 채널)
//    지역의 상위값(예: "서울")은 "서울"과 "서울 강남구"처럼 같은 시·도의
//    상세 지역을 함께 고른다. 상세 지역·경력 조건·채널은 완전일치로 비교한다.
//    value가 빈 문자열("전체" 선택)이면 필터를 적용하지 않고 그대로 돌려준다.
// ---------------------------------------------------------------------------

function filterByRegion(rows, value) {
  if (!value) return rows;
  return rows.filter((row) => matchesRegionValue(row.region, value));
}

function filterByCategory(rows, value) {
  if (!value) return rows;
  return rows.filter((row) => matchesFilterValue(row.category, value));
}

function filterBySource(rows, value) {
  if (!value) return rows;
  return rows.filter((row) => matchesFilterValue(row.source, value));
}
...
function matchesRegionValue(rowValue, selectedValue) {
  if (selectedValue === MISSING_FILTER) return rowValue.trim() === '';
  if (selectedValue.includes(' ')) return rowValue === selectedValue;
  return rowValue === selectedValue || rowValue.startsWith(`${selectedValue} `);
}
...
// ---------------------------------------------------------------------------
// 5. 브라우저 전용 구간 (렌더링 + 이벤트 + 초기 로드)
//    document가 없는 환경(Node 등)에서는 이 블록 전체를 건너뛴다.
//    -> 위의 순수 함수들만 따로 불러와 단위 검증할 수 있다.
// ---------------------------------------------------------------------------
...
if (typeof document !== 'undefined') {
  ...
  async function init() {
    ...
    originalRows = parseCSV(text);         // 원본 - 이후 절대 재할당하지 않는다
    cleanedRows = cleanData(originalRows);  // D03 산출물 - 새 배열
    filteredRows = cleanedRows;             // 초기 상태 = 필터 없음("전체")
